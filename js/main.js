...
async function init() {
    ...
    try {
        const [scores, user, recent] = await Promise.all([
            fetch('/api/scores?type=best').then(r => r.ok ? r.json() : []),
            fetch('/api/scores?type=user').then(r => r.ok ? r.json() : null),
            fetch('/api/scores?type=recent').then(r => r.ok ? r.json() : [])
        ]);

        if (user) renderUser(user);
        if (scores.length) renderScores(scores);
        if (recent.length) renderRecent(recent);
        
    } catch (e) {
        document.getElementById('live-stats-container').innerHTML = '<div class="text-red-400 text-xs text-center p-4">Failed to load API data</div>';
    }
...
function renderUser(u) {
    ...
    if (u.rank_history && u.rank_history.length > 1) {
...
function drawRankGraph(history) {
    ...
    history.forEach((val, i) => {
        ...
        const y = ((val - minRank) / range) * (h - 20) + 10; 
...
function renderScores(data) {
    ...
    const createRow = (s) => {
        ...
        <div class="score-row group relative w-full bg-[#121214] border border-white/5 rounded-xl overflow-hidden mb-2" onclick="this.classList.toggle('open')">
            <div class="relative h-20 w-full flex items-center cursor-pointer z-10">
                ...
                <div class="relative z-10 flex-grow px-4 flex flex-col justify-center min-w-0">
                    <span class="text-sm font-bold text-white truncate block">${s.beatmap.title}</span>
                    <div class="text-[10px] text-gray-400 truncate">${s.beatmap.version} <span class="ml-2">${mods}</span></div>
                </div>
    ...
    c.innerHTML = `<h3 class="text-xs font-bold uppercase text-gray-500 mb-3 mt-6">Top Scores</h3>` + data.slice(0, 5).map(createRow).join('');
    ...
    if (data.length > 5) {
        ...
        btn.onclick = () => { c.innerHTML += data.slice(5).map(createRow).join(''); btn.remove(); };
        c.appendChild(btn);
    }
}
...
function renderRecent(data) {
    ...
    div.className = "mt-6 bento-card p-4";
    div.innerHTML = `<h3 class="text-xs font-bold uppercase text-gray-500 mb-3">Recent Activity</h3>` + 
        data.slice(0,5).map(s => `
        <div class="flex justify-between items-center py-2 border-b border-white/5 last:border-0">
            <div class="flex items-center gap-3"><span class="text-xl font-bold rank-${s.rank} w-8 text-center">${s.rank.replace('X','SS')}</span> <span class="text-xs text-white truncate max-w-[200px] md:max-w-xs">${s.beatmap.title}</span></div>
            <div class="text-[10px] text-gray-500">${Utils.timeAgo(s.created_at || s.date_iso)}</div>
        </div>`).join('');
...
async function fetchLanyard() {
    try {
        ...
        if (osu) {
            w.className = "inline-flex items-center rounded-full border mb-6 px-4 py-1 bg-pink-500/10 border-pink-500/30 text-pink-400 text-xs font-bold backdrop-blur-md";
            w.innerHTML = `Playing osu!`;
        } else {
            const color = data.discord_status === 'online' ? 'green' : 'gray';
            w.className = `inline-flex items-center rounded-full border mb-6 px-3 py-1 bg-${color}-500/10 border-${color}-500/30 text-${color}-400 text-xs font-bold backdrop-blur-md transition-colors`;
