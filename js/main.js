...
const Utils = {
    ...
    calculateMapStats: (b, mods) => {
        let cs = b.cs, ar = b.ar, od = b.od || b.accuracy, hp = b.hp || b.drain, bpm = b.bpm, len = b.length || b.total_length;
...
// Определяем функции глобально (window), чтобы они были доступны друг другу
window.loadTopScores = async function() {
    ...
    try {
        const res = await fetch('/api/scores?type=best'); // Изменил путь для Vercel API
        ...
        <div id="score-row-${i}" class="score-row group relative w-full bg-[#121214] border border-white/5 rounded-xl overflow-hidden transition-all duration-300">
            <div onclick="toggleScoreDetails(${i})" class="relative h-20 w-full flex items-center cursor-pointer z-10 bg-[#121214]">
                ...
                <div class="relative z-10 flex-grow min-w-0 px-4 py-2 flex flex-col justify-center h-full">
                    ...
                    <div class="text-[10px] text-gray-400 truncate mb-1">by ${s.beatmap.artist || 'Artist'} <span class="text-gray-600 mx-1">•</span> <span class="text-gray-300">[${s.beatmap.version}]</span></div>
...
// Теперь функция определена глобально и будет доступна
window.loadRecentActivity = async function() {
    const c = document.getElementById('activity-container');
    if (!c) return;
    try {
        const res = await fetch('/api/scores?type=recent'); // API path fixed
        if (!res.ok) throw new Error();
...
window.loadUserStats = async function() {
    ...
    if (!c) return;
    try {
        const res = await fetch('/api/scores?type=user'); // API path fixed
        if (!res.ok) throw new Error();
